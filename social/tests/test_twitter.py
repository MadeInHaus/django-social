...
class TwitterTest(TestCase):
    ...
    def test_twitter_model_create_from_obj(self):
    ...
    def test_twitter_api_get_user_timeline(self):
        ...
        tweets = twapi.get_user_timeline('dinopetrone')
        try:
            for tweet in tweets:
                try:
    ...
    def test_twitter_api_search(self):
        ...
        twapi = TwitterAPI( 
            client_key = settings.SOCIAL_TWITTER_CONSUMER_KEY, 
            client_secret = settings.SOCIAL_TWITTER_CONSUMER_SECRET,
            resource_owner_key = '295947098-NSoaKeNt5sTNpCC1SreUHY9lTbGTlfxEHiIxBPAJ',
            resource_owner_secret = 'HX80zECI1H1ruSFJSQ074bWB6TU9vonMFPgtzmUbY')

        tweets = twapi.search('#test')
        tweet_duplicate = 0
        try:
            for tweet in tweets:
                try:
